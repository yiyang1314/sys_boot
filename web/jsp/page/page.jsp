...
<body>
	...
	<form name="userform" id="userform" action="${param.action}" >
		...
		<p>
			查询列名：
				<select class="column"  id="column" name="column">
					<c:forEach var="col" items="${columns}">
						<option ${param.column==col.key?'selected':''} value="${col.key}" >${col.value}</option>
					</c:forEach>
				</select>
				关键字：<input id="keywords" name="keywords" type="text"  value="" />
				<input type="button" name="search" value="搜索" onclick="nextPage('1')"/>
			</p>
		</c:if>
	</form>
	</c:if>
